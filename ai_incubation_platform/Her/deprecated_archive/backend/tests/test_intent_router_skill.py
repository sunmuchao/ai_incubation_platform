...
- v3.0: 适配 YAML 配置驱动版本（仅 7 种核心意图）

注意：新架构简化为 7 种意图：
- matching, icebreaker, compatibility_analysis, date_planning, profile_collection
- greeting, general（本地意图）

以下意图已废弃删除，相关测试已跳过：
- daily_recommend（合并到 matching）
- gratitude, goodbye, capability_inquiry（合并到 general）
- pre_communication, pre_comm_start（使用 DeerFlow her_pre_comm_tool）
- topic_suggestion, dating_suggestion（合并到 icebreaker/date_planning）
"""
import sys
import os
...
# 标记已废弃意图的测试
DEPRECATED_INTENTS = [
    "daily_recommend", "gratitude", "goodbye", "capability_inquiry",
    "pre_communication", "pre_comm_start", "topic_suggestion", "dating_suggestion",
...
class TestIntentConfigLoader:
    ...
    def test_matching_intent_maps_to_deerflow_tool(self):
        """测试匹配意图映射到 her_find_matches DeerFlow 工具"""
        sorted_intents = self.config.get_sorted_intents()
        matching_intent = None
        for intent in sorted_intents:
            if intent.name == "matching":
                matching_intent = intent
                break
        assert matching_intent is not None
        # 新架构使用 deerflow_tool 字段
        deerflow_tool = getattr(matching_intent, 'deerflow_tool', None)
        assert deerflow_tool == "her_find_matches"
    ...
    def test_gratitude_is_local_intent(self):
        """测试感谢意图 - 新架构中 gratitude 已移除，使用 general 处理"""
        # 新架构只有 7 个意图，gratitude 已合并到 general
        local_intents = self.config.get_local_intents()
        assert "greeting" in local_intents or "general" in local_intents

    def test_capability_inquiry_is_local_intent(self):
        """测试能力询问 - 新架构中 capability_inquiry 已移除，使用 general 处理"""
        # 新架构只有 7 个意图，capability_inquiry 已合并到 general
        local_intents = self.config.get_local_intents()
        assert "general" in local_intents
...
class TestKeywordIntentClassification:
    ...
    def test_classify_greeting_variants(self):
        """测试各种问候变体 - 仅测试配置中存在的关键词"""
        # 新配置只包含: 你好, hi, hello, 嗨
        inputs = ["hi", "hello", "嗨"]
        for input in inputs:
            result = self.skill._classify_with_keywords(input)
            assert result.type == IntentType.GREETING
        # "早上好" 不在配置中，会匹配到 GENERAL
        result = self.skill._classify_with_keywords("早上好")
    ...
    @pytest.mark.skip(reason="daily_recommend 已合并到 matching")
    ...
    @pytest.mark.skip(reason="gratitude 已合并到 general")
    ...
    @pytest.mark.skip(reason="goodbye 已合并到 general")
    ...
    @pytest.mark.skip(reason="capability_inquiry 已合并到 general")
    ...
    @pytest.mark.skip(reason="pre_communication 使用 DeerFlow 工具")
    ...
    @pytest.mark.skip(reason="pre_comm_start 使用 DeerFlow 工具")
    ...
    @pytest.mark.skip(reason="topic_suggestion 已合并到 icebreaker")
    ...
    @pytest.mark.skip(reason="dating_suggestion 已合并到 date_planning")
...
class TestLocalIntentHandling:
    ...
    def test_feature_list_count(self):
        """测试功能列表数量 - 新架构只有 4 个功能"""
        features = self.skill._get_feature_list()
        assert len(features) >= 4  # 新架构: 智能匹配、破冰建议、匹配度分析、约会策划
...
class TestSuggestedActions:
    ...
    @pytest.mark.skip(reason="新架构不使用 suggested_actions")
...
class TestIntentRouterSkillExecute:
    ...
    @pytest.mark.skip(reason="新架构使用 DeerFlow 工具而非 Skill Registry")
    async def test_execute_matching_calls_matchmaker_skill(self):
        """测试匹配意图调用 ConversationMatchmaker Skill - 已废弃"""
        skill = get_intent_router_skill()
...
class TestSkillRegistryIntegration:
    ...
    def test_intent_router_registered(self):
        """测试 IntentRouter 已注册 - 新架构不使用 Registry"""
    ...
    def test_intent_router_has_entry_point_tag(self):
        """测试 IntentRouter 有 entry_point 标签 - 新架构不使用 Registry"""
        from agent.skills.registry import get_skill_registry, initialize_default_skills
