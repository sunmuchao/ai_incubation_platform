...
@deprecated
此服务已废弃，原因：
1. /api/profile/quickstart 端点直接操作数据库，不调用此 Service
2. 形成逻辑孤岛链：vector_adjustment_service → quick_start_service → 无入口
3. 灰度检查端点只读取配置，不调用此 Service

替代方案：
- 前端直接调用 /api/profile/quickstart/submit 端点
- 端点内部通过 _update_user_profile() 直接操作数据库
- 画像收集改用 DeerFlow profile_collection_skill
