...
@deprecated
此服务已废弃，原因：
1. 无任何活跃入口（API/Skills/main.py 都不调用）
2. 功能已迁移到 DeerFlow her_tools 动态画像工具
3. 只被 services/__init__.py 导出，无实际使用

替代方案：
- DeerFlow Agent 通过 her_dynamic_profile_tool 动态更新画像
- 前端调用 /api/deerflow/chat 端点触发画像分析
