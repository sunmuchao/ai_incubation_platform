...
from db.models import (
    ...
    MatchHistoryDB, ChatMessageDB, UserReportDB
)
...
class SafetyAIService:
    ...
    def _get_sender_risk_profile(self, user_id: str) -> Dict[str, Any]:
        ...
        # 查询用户被举报次数（已确认的举报）
        report_count = self.db.query(UserReportDB).filter(
            UserReportDB.reported_user_id == user_id,
            UserReportDB.status.in_(["approved", "processed"])
        ).count()

        # 查询待审核的举报数量（高优先级）
        pending_high_priority_reports = self.db.query(UserReportDB).filter(
            UserReportDB.reported_user_id == user_id,
            UserReportDB.status == "pending",
            UserReportDB.priority >= 4
        ).count()

        # 计算基础风险分
        # 违规记录贡献：每次 10 分，最高 30 分
        violation_score = min(30, risk_events * 10)
        # 已确认举报贡献：每次 15 分，最高 45 分
        report_score = min(45, report_count * 15)
        # 高优先级待审核举报：每次 10 分，最高 25 分
        pending_score = min(25, pending_high_priority_reports * 10)

        base_risk = violation_score + report_score + pending_score
        ...
        return {
            ...
            "pending_high_priority_reports": pending_high_priority_reports,
    ...
    def _check_profile_authenticity(self, user: UserDB) -> Dict[str, Any]:
        ...
        # 检查照片验证状态
        # 注：当前简化处理，生产环境应对接照片验证服务
        photo_verified = getattr(user, "photo_verified", False)
        if not photo_verified:
            authenticity_score -= 25
            issues.append("photo_not_verified")
        else:
            logger.info(f"SafetyAIService: User {user.id} photo verified")

        # 检查实名认证状态
        # 注：当前简化处理，生产环境应对接实名认证服务
        real_name_verified = getattr(user, "real_name_verified", False)
        if not real_name_verified:
            authenticity_score -= 20
            issues.append("real_name_not_verified")
        else:
            logger.info(f"SafetyAIService: User {user.id} real name verified")
