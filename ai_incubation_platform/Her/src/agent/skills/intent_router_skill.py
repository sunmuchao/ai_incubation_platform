...
重构说明：
- 移除 INTENT_KEYWORDS, INTENT_PRIORITY, INTENT_SKILL_MAPPING 等硬编码
- 改用 config/intents.yaml 配置文件
- 支持热更新（修改 YAML 无需重启服务）
"""
...
class IntentRouterSkill(BaseSkill):
    ...
    async def execute(
        ...
        **kwargs
        ...
        logger.info(f"[IntentRouter] 分析用户输入: {user_input[:50]}...")
