...
class DateCoachSkill(BaseSkill):
    ...
    def _review_date(
        ...
        # 尝试 AI 分析
        ai_review = self._ai_review_date(conversation_history, date_context)
        if ai_review:
            return ai_review

        # 降级：基于规则的简单分析
        return self._fallback_review_date(conversation_history)
    ...
    def _ai_review_date(
        ...
        try:
            ...
            # 提取对话内容
            messages_text = "\n".join([
                f"{msg.get('sender', '对方')}: {msg.get('content', '')}"
                for msg in conversation_history[-20:]  # 最近 20 条
            ])

            prompt = f'''你是一位约会教练，请分析以下约会对话并生成复盘报告。
...
请返回 JSON 格式：
{{
    "overall_rating": "很好/不错/一般/待改进",
    "summary": "一句话总结约会整体情况",
    "highlights": ["亮点1", "亮点2"],
    "areas_to_improve": ["改进点1", "改进点2"],
    "next_steps": ["下一步建议1", "下一步建议2"],
    "mood_analysis": {{
        "positive_moments": 数字,
        "concerning_moments": 数字
    }}
}}
...
只返回 JSON，不要其他内容。'''
            ...
            if response and not response.startswith('{"fallback"'):
                ...
                return {
                    "overall_rating": data.get("overall_rating", "不错"),
                    "summary": data.get("summary", ""),
                    "positive_moments": data.get("mood_analysis", {}).get("positive_moments", 0),
                    "concerning_moments": data.get("mood_analysis", {}).get("concerning_moments", 0),
            ...
            logger.debug(f"DateCoachSkill: AI review failed: {e}")
            return None

    def _fallback_review_date(self, conversation_history: List[Dict]) -> Dict[str, Any]:
        """
        降级：基于规则的约会复盘
        ...
        for msg in conversation_history:
            ...
            try:
                result = analyze_text_emotion_sync(content)
