...
class MatchmakerAlgorithm:
    ...
    def _check_basic_compatibility(self, user: dict, candidate: dict) -> bool:
        ...
        # 性别偏好检查
        if user.get('preferred_gender') and candidate.get('gender') != user['preferred_gender']:
            return False
        if candidate.get('preferred_gender') and user.get('gender') != candidate['preferred_gender']:
            return False

        # 地点偏好检查（简化）
        if user.get('preferred_locations') and candidate.get('location') not in user['preferred_locations']:
            if candidate.get('preferred_locations') and user.get('location') not in candidate['preferred_locations']:
                return False
