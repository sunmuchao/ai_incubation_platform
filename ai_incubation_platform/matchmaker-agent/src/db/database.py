...
# 连接池配置（生产环境优化）
POOL_SIZE = int(settings.database_pool_size) if hasattr(settings, 'database_pool_size') else 10
MAX_OVERFLOW = int(settings.database_max_overflow) if hasattr(settings, 'database_max_overflow') else 20
POOL_TIMEOUT = int(settings.database_pool_timeout) if hasattr(settings, 'database_pool_timeout') else 30
