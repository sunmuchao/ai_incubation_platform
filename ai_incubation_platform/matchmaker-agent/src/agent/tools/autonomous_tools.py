...
class CompatibilityAnalysisTool:
    ...
    def _analyze_personality(user1, user2) -> tuple:
        ...
        # 获取用户性格特征
        cursor.execute("SELECT personality_traits FROM user_memberships WHERE user_id = ?", (user1.id,))
        row1 = cursor.fetchone()
        cursor.execute("SELECT personality_traits FROM user_memberships WHERE user_id = ?", (user2.id,))
        row2 = cursor.fetchone()

        traits1 = json.loads(row1[0]) if row1 and row1[0] else {}
        traits2 = json.loads(row2[0]) if row2 and row2[0] else {}

        if not traits1 or not traits2:
            # 如果没有性格数据，基于行为推断
            return CompatibilityAnalysisTool._infer_personality_from_behavior(user1, user2)
        ...
        for trait in big5_traits:
            ...
            # 互补型或相似型匹配（取决于特质）
            if trait in ["extraversion", "neuroticism"]:
                # 互补型：外向 - 内向可能互补
                diff = abs(val1 - val2)
                score = 1.0 - (diff * 0.5)  # 允许一定差异
            else:
                # 相似型：价值观等越相似越好
                diff = abs(val1 - val2)
                score = 1.0 - diff
        ...
        details = {
            "big5_scores": dict(zip(big5_traits, [round(s, 2) for s in scores])),
...
class TopicSuggestionTool:
    ...
    def handle(
        ...
        try:
            ...
            # 获取匹配信息
            cursor.execute("""
                SELECT user_id_1, user_id_2, common_interests, relationship_stage
                FROM match_history
                WHERE id = ?
            """, (match_id,))
            ...
            user_id_1, user_id_2, common_interests_str, relationship_stage = match_row
...
class RelationshipTrackingTool:
    ...
    def handle(
        ...
        try:
            ...
            # 获取匹配信息
            cursor.execute("""
                SELECT user_id_1, user_id_2, relationship_stage, created_at
                FROM match_history
                WHERE id = ?
            """, (match_id,))
            ...
            user_id_1, user_id_2, current_stage, created_at = match_row
    ...
    def _collect_interactions(db, user_id_1: str, user_id_2: str, period: str) -> dict:
        ...
        cursor.execute("""
            ...
            AND created_at >= ?
        """, (user_id_1, user_id_2, user_id_2, user_id_1, since))
        message_count = cursor.fetchone()[0]

        # 获取活跃天数
        cursor.execute("""
            SELECT COUNT(DISTINCT DATE(created_at)) FROM chat_messages
            WHERE ((sender_id = ? AND receiver_id = ?)
               OR (sender_id = ? AND receiver_id = ?))
            AND created_at >= ?
        """, (user_id_1, user_id_2, user_id_2, user_id_1, since))
