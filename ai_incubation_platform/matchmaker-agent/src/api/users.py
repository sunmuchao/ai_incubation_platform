...
from models.user import User, UserCreate, UserProfile, UserUpdate, Gender, RelationshipGoal
...
async def login(credentials: UserCredentials, service=Depends(get_user_service)):
    ...
    if not user or not user.password_hash:
        logger.warning(f"Login failed: user not found for username: {credentials.username}")
        raise HTTPException(
