...
async def get_matches(
    ...
    - 限流保护（50 次/秒突发）
    """
    logger.info(f"Getting matches for user: {user_id}, limit: {limit}")
    ...
    if not db_user:
        logger.warning(f"Get matches failed: user not found: {user_id}")
...
async def get_recommendations(
    ...
    for user in all_users:
        ...
        # 检查认证状态
        identity_db = get_db().execute(
            "SELECT * FROM identity_verifications WHERE user_id = ? AND verification_status = 'verified'",
            (user.id,)
        ).fetchone()
