...
class BehaviorTrackingService:
    ...
    def __init__(self):
        ...
        self._buffer_size_limit = 100  # 缓冲区大小限制
        self._buffer_time_limit = timedelta(minutes=5)  # 缓冲区时间限制
    ...
    def flush_events(self, user_id: str) -> int:
        ...
        try:
            ...
            db.rollback()
            logger.error(f"Error flushing behavior events: {e}")
            # 重新加入缓冲区
            self._event_buffer[user_id].extend(events)
            return 0
        finally:
            db.close()
    ...
    def analyze_preference_shift(
        ...
        try:
            ...
            # 分析年龄偏好变化
            age_preferences = self._analyze_age_preference_shift(events)

            # 分析地点偏好变化
            location_preferences = self._analyze_location_preference_shift(events)

            # 分析类型偏好变化（通过 like/pass 行为）
            type_preferences = self._analyze_type_preference_shift(events)
    ...
    def _analyze_age_preference_shift(self, events: List[BehaviorEventDB]) -> Dict[str, Any]:
        """分析年龄偏好变化"""
        # 简化实现：基于 like 行为分析
        from db.database import SessionLocal as get_db
    ...
    def _analyze_location_preference_shift(self, events: List[BehaviorEventDB]) -> Dict[str, Any]:
        """分析地点偏好变化"""
        # 简化实现
        return {"shift": False, "description": ""}

    def _analyze_type_preference_shift(self, events: List[BehaviorEventDB]) -> Dict[str, Any]:
        """分析类型偏好变化"""
