...
if [[ $isOnlyHihidataChange == "false" ]]; then
        ...
        docker exec $containerName /bin/bash -c '/usr/lib/foundationdb/fdbmonitor --conffile /etc/foundationdb/foundationdb.conf --lockfile /var/run/fdbmonitor.pid --daemonize && mvn clean package -Dmaven.test.skip=true && mvn test surefire-report:report site -Dmaven.test.failure.ignore=true'
elif [[ $isOnlyHihidataChange == "true" ]];then
        docker exec $containerName /bin/bash -c '/usr/lib/foundationdb/fdbmonitor --conffile /etc/foundationdb/foundationdb.conf --lockfile /var/run/fdbmonitor.pid --daemonize && mvn clean package -Dmaven.test.skip=true && mvn test -pl polars-hihidata surefire-report:report site -Dmaven.test.failure.ignore=true'
fi